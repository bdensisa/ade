...
def update_courses(events, user_id):
    ...
    mycursor = mydb.cursor()
    mycursor.execute(
        "DELETE FROM UserCourses WHERE user_id = %s AND id NOT IN (%s)",
    ...
    mycursor = mydb.cursor()
    sql = "INSERT INTO UserCourses (id, user_id, title, start, end, location, description) VALUES (%s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE title = VALUES(title), start = VALUES(start), end = VALUES(end), location = VALUES(location), description = VALUES(description)"
    values = [
